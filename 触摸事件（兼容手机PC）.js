...
+ function($) {
  ...
  $.toast = function(text, style ,className) {
    if(style){
      show('<i class="weui_icon_toast"></i><p class="weui_toast_content">' + (text || "已经完成！") + '</p>',className);
    }else{
      show('<i class="weui_icon_cancel"></i><p class="weui_toast_content">' + (text || "操作失败！") + '</p>',className);
    }
  ...
  //新增 --toast
  $('body').on('click','.weui_toast',hide);
  $('body').on('click','.weui_mask_transparent',hide);
